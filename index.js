...
module.exports = (api, options) => {
  ...
  api.registerCommand(
    'leanix-serve',
    {
      description: 'Launches LeanIX dev server',
      usage: 'vue-cli-service leanix-serve',
      options: {
        '--open': 'open browser window on server start',
    ...
    async function serve (args) {
      const { open, copy, mode, host, port } = args
      info('Getting access token')
      const leanixReporting = new LeanixReporting(port)
      const launchURL = leanixReporting.getLaunchURL()
...
/*
      leanixReporting.getLaunchURL().then(launchURL => {
        console.log('launch url', launchURL)
      })
      console.log(`👋 Launching leanix dev server`, leanixReporting)
    }
  )
}
*/
